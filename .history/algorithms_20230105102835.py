...
def dijkstra(graph, source, target):
    ...
    while True:
        ...
        for neighbor in nachbarn:
            others = [item in nachbarn if item != neighbor]
            for item in others:
                if neighbor in graph.neighbors(item):
                    try:
                        relative_cost = distances[neighbor] + weights[(item, neighbor)]
                    except:
                        relative_cost = distances[neighbor] + weights[(neighbor, item)]
    ...
    while target:
        ...
        cost += dist
        target = dest
    path.append(graph.vs[source]['name'])
    path = ' -> '.join(list(reversed(path)))
    return path, cost, distances

# there's a problem with distance finding. try finding it. this should help: https://youtu.be/bZkzH5x0SKU
