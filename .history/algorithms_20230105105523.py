...
def dijkstra(graph, source, target):
    ...
    while True:
        ...
        for neighbor in nachbarn:
            try:
                if distances[neighbor] in [[float('inf'), node], [float('inf'), None]]:
                    distances[neighbor][0] = weights[(node, neighbor)]
                elif distances[neighbor][1] == node:
                    distances[neighbor][0] += weights[(node, neighbor)]
    ...
    named = {graph.vs[k]['name']: [v[0], graph.vs[[v[1]]]['name']] for k, v in distances.items()}
    return named
