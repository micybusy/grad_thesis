...
def ddfs(graph, v, tail = [], visit_time = {}, counter = 0):
    vertices = [vertex.index for vertex in graph.vs]
    if v not in tail:
        nachbarn = graph.neighbors(v, 'out')
        tail.append(v)
        for ix, vertex in enumerate(nachbarn):
            if vertex not in visit_time:
                visit_time[vertex] = counter+ix+1
            ddfs(graph, vertex, tail, visit_time, counter= counter)
        counter +=1 
    if set(tail) == set(vertices):
        return tail, visit_time
    else:
...
#print(ddfs(scc(), 0))
str_dfs = ddfs(wcomplex3(directed= True), 0)
print(str_dfs)
plotter(wcomplex3(directed= True))
