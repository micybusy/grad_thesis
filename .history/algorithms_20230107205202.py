...
def ddfs(graph, v, tail = [], counter = 0, fin_times = {}):
    print(counter)
    vertices = [vertex.index for vertex in graph.vs]
    if v not in tail:
        nachbarn = graph.neighbors(v, 'out')
        if all(x in tail for x in nachbarn):
            fin_times[v] = counter
        tail.append(v)
        for vertex in nachbarn:    
            ddfs(graph, vertex, tail, counter+1, fin_times)

    if set(tail) == set(vertices):
        return tail, fin_times
    else:
