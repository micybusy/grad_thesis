    ...
    for vertex in vertices:
        for vertex2 in vertices:
            if vertex != vertex2:
                x = graph.copy()
                try:
                    x.delete_edges((vertex, vertex2))
                except:
                    x.delete_edges((vertex2, vertex))
                else:
                    pass
                
                if not x.is_connected():
                    possibles.append((vertex, vertex2))
                

    return vertices, possibles

# dfs is working fine. now implement articulation point & biconnected components algorithm. https://youtu.be/jFZsDDB0-vo

print(articulation_point(wcomplex()))
plotter(wcomplex(), weighted= False)