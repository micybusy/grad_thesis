...
#print(ddfs(scc(), 0))
str_dfs = dfs(wcomplex3(directed= False), 0, tail = [], weighted= False)
print(str_dfs)
#plotter(wcomplex3(directed= False), weighted=False)

x = [(edge.target, edge.source) for edge in str_dfs[1].es()]
x = ig.Graph(edges = x, directed = True)
x.vs['name'] = [str(vertex.index) for vertex in x.vs]
plotter(x)

plotter(wcomplex3(directed= False))
