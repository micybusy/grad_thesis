...
def dfs(graph, v,  tail = []):
    ...
    dfs_tree.vs['name'] = [graph[vertex]['name'] for vertex in dfs_tree.vs]
    for edge in dfs_tree.es:
        certain = [item in graph.es if item.source, item.target == edge.source, edge.target]
        edge['weight'] = certain['weight']
    return path
...
#plotter(wcomplex2(), weighted= False)
#cost, mst, mst_named = kruskal(wcomplex())
#print(mst_named)
#plotter(mst, weighted= False)
print(dfs(wcomplex3(), 0))
plotter(wcomplex3(), weighted= False)
