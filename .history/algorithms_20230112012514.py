...
def ddfs(graph, v, tail = [], visit_time = {}, counter = 1):
    ...
    if v not in tail:
        ...
        if not nachbarn:
            ...
            for node in backtrack:
                sub_nach = graph.neighbors(v, 'out')
                if all([(item in tail) for item in sub_nach]):
                    counter+=1
                    visit_time[node] = counter
        for vertex in nachbarn:
            ddfs(graph, vertex, tail, visit_time, counter+1)
